...
namespace Microsoft.Framework.DependencyInjection
{
    ...
    public static class EntityServiceCollectionExtensions
    {
        ...
        ///         from an external <see cref="IServiceProvider"/>. If you are not using an external <see cref="IServiceProvider"/>
        ...
        ///         when using EntityFramework.SqlServer you would call <c>collection.UseEntityFramework(config).UseSqlServer()</c>.
        ///     </para>
        ///     <para>
        ///         For derived contexts to resolve their services from the <see cref="IServiceProvider"/> you must chain a call to 
        ///         the <see cref="AddDbContext{TContext}"/> extension method on the returned <see cref="EntityFrameworkServicesBuilder"/>.
        ///         This will ensure services are resolved from the <see cref="IServiceProvider"/> and any Entity Framework configuration
        ///         from the supplied <paramref name="configuration"/> will be honored.
        ///     </para>
        /// </summary>
        ...
        ///         For this configuration to be applied you must register any derived contexts using the <see cref="AddDbContext{TContext}"/> 
        ///         extension method on the returned <see cref="EntityFrameworkServicesBuilder"/>.
        ///     </para>
        /// </param>
        /// <returns> 
        ...
        /// <summary>
        ///     Registers the given context as a service in the <see cref="IServiceCollection"/>. This will ensure services that the 
        ///     context uses are resolved from the <see cref="IServiceProvider"/> and any Entity Framework configuration found in the
        ///     configuration passed to <see cref="AddEntityFramework"/> will be honored.
        /// </summary>
        ...
        ///         to performing configuration of the context by overriding the <see cref="DbContext.OnConfiguring"/> method in your
        ///         derived context.
        ///     </para>
        ///     <para>
        ///         If an action is supplied here, the <see cref="DbContext.OnConfiguring"/> method will still be run if it has been
        ///         overridden on the derived context. <see cref="DbContext.OnConfiguring"/> configuration will be applied in addition 
        ///         to configuration performed here.
        ///     </para>
        ///     <para>
        ///         You do not need to expose a constructor parameter for the <see cref="DbContextOptions"/> to be passed to the context.
        ///         If you choose to expose a constructor parameter, you must type it as the generic <see cref="DbContextOptions{T}"/>
        ///         as that is the type that will be registered in the <see cref="IServiceCollection"/> (in order to support multiple
        ///         context types being registered in the same <see cref="IServiceCollection"/>).
        ///     </para>
        /// </param>
        /// <returns>
        ///     A builder that allows further Entity Framework specific setup of the <see cref="IServiceCollection"/>. 
